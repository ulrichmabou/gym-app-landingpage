  ...
  let productFeatures = [
    ...
    },
    {
      featureList: [
        ...
        "Guarentees progress on all core exercises",
      ],
      description:
...
<SectionWrapper id="project">
  <div
    class="flex flex-col gap-10 sm:gap-14 md:gap-24 flex-1 items-center
  ...
  >
    <div class="flex flex-col gap-2">
      ...
      <h3
        class="text-4xl sm:text-5xl md:text-6xl max-w-[1000px] mx-auto w-full
      ...
      <ProductCard {productFeature} {index}>
        {#if index === 0}
          Experience a beginner exercise <span
            class="text-indigo-400 tont-medium">master</span
          > class
        {:else if index === 1}
        ...
        {:else}
          Workouts specific to your <span class="text-indigo-400 font-medium"
            >environment</span
          >
          and <span class="text-indigo-400 font-medium">expertise</span>
        {/if}
      </ProductCard>
    {/each}
  </div>
